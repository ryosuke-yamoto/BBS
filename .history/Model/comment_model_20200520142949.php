<?php
  ...
  function create_comment_list($pdo, $article_id) {//コメント一覧の配列を生成します
    ...
    try{
      ...
      unset($row);
      //$comment_arrayの中から$article_idと一致する配列を取得しこのページで表示するコメントの配列（$this_comment）を生成しま
      // $this_comment = (array_filter($comment_array, function($row) {
      //   return $row["article_id"] == $article_id;
      // },ARRAY_FILTER_USE_BOTH));
      foreach($comment_array as $comment) {
        if($comment["article_id"] == $article_id) {
          $this_comment = array_merge($this_comment, $comment);
        };
      };
      unset($comment);
