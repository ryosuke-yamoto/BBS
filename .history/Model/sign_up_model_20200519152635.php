<?php
  ...
  function register_user_temporery($mail,$token,$pdo) {//メール入力判定並びに仮登録及びメール送信
    global $e;
      ...
      //       $_SESSION["error"]["mail_registered"] = "このメールアドレスはすでに登録されてあります。";
      //       return 1;
      //     };
      //   };
  ...
  function sign_up_user($mail, $password, $name, $token, $pdo) {
    try {
  ...
  function delete_user_temporary() {//新規登録できたら仮登録のデータを論理削除
    try {
      
      
      $sql = "update php_task.user_temporary set deleted = now() where urltoken = :token;";
      $stmt = $pdo -> prepare($sql);
