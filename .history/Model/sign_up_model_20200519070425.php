<?php
  ...
  function register_user_temporery($mail,$token,$pdo) {
    global $e;
      ...
      try {
        $sql = "SELECT email, iv FROM php_task.user_temporary";  
        ...
        foreach ($stmt as $row) {
          ...
          echo $row["iv"] . "iv</br >";
          var_dump(hex2bin($row["email"]) . "bex2bin</br >");



          $encrypted = openssl_encrypt($row["email"], 'aes-256-cbc', "password1234", 0, hex2bin($row['iv']));
