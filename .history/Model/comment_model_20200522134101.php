<?php
  ...
  function post_comment($content, $user_id,$article_id, $pdo) {//コメントを投稿します}
    //入力フォームチェック
    if ($content == ""){
      return $_SESSION["error"] = "コメント内容が入力されていません。";
    };
  ...
  function create_comment_list($pdo, $article_id) {//コメント一覧の配列を生成します
    ...
    try{
      //DBからarticleを取り出す
      $sql = "SELECT comment_id, content, created, user_id, article_id FROM php_task.comment WHERE deleted IS NULL order by created asc;";
      $stmt = $pdo->prepare($sql);
      $stmt->execute();
      ...
      $_SESSION["error"] = "記事の取得に失敗しました。";
    };
  };

  function delete_comment($comment_id, $pdo) {//コメントを削除します
    try{
      $sql = "UPDATE comment SET deleted = now() WHERE comment.comment_id = :comment_id;";
      $stmt = $pdo -> prepare($sql);
      $stmt->bindValue(":comment_id", $comment_id, PDO::PARAM_STR);
      $stmt->execute();
    }catch (PDOException $e){
