<?php
  ...
  function sign_up_user($mail, $password, $name, $token, $pdo) {//新規登録及び仮登録のデータを論理削除
    try {
      ...
      $mail_stmt->execute();
      $mail_data = $mail_stmt->fetch(PDO::FETCH_ASSOC);
      $decrypted_mail = openssl_decrypt($mail_data["email"], 'aes-256-cbc', "password1234", 0, hex2bin($mail_data["iv"]));
      var_dump($decrypted_mail);
      exit;
