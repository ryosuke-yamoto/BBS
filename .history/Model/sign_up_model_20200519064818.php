<?php
  ...
  function register_user_temporery($mail,$token,$pdo) {
    global $e;
      ...
      try {
      //   $sql = "SELECT email FROM php_task.user_temporary";  
      //   $stmt = $pdo -> prepare($sql);
      //   // $stmt->bindValue(":email", $mail, PDO::PARAM_STR);
      //   $stmt->execute();
      //   foreach ($stmt as $row) {
      //     $encrypted = openssl_encrypt($row[' email'], 'aes-256-cbc', $password, $options, $iv);
      //     $ = $row['id'];
      //     echo $id;
      //   }
      //   //レコード件数取得
    ...
    //１回目のtryですでにメールアドレスがあったら登録できない
      ...
      $sql = "INSERT INTO php_task.user_temporary(email, urltoken, created, iv) VALUES(:email, :urltoken, :iv, now())";
