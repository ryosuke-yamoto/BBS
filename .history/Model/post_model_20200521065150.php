<?php
  ...
  function post_article($title, $content, $pdo) {//新規投稿します
    try{
      ...
      $sql = "INSERT INTO php_task.article(article_id, title, content, created, user_id ) VALUES(:article_id, :title, :content, date_format(now(), '%Y年%m月%d日 %H時%i分%s秒'), :user_id)";
  ...
  function update_article($update_id, $update_title, $update_content, $pdo) {
    try{
      ...
      $sql = "UPDATE php_task.article SET title=:title, content=:content, updated=now() WHERE article.article_id = :article_id;";
      $stmt = $pdo->prepare($sql);
