<?php
  ...
  if(!isset($_SESSION["user_id"])) {
    header("Location: http://localhost:8888/BBS_yamoto/Controller/auto_signin.php");
  };
  // setcookie("auto_signin" , "" , time()-100 );
  $cookieExpire = time() + 3600 * 24 * 7; // 7日間
  if(empty($_COOKIE["auto_signin"])) {
    setcookie("auto_signin", $_SESSION["auto_signin"], $cookieExpire);
  }else {
    //ログインするたびに再生成
    $_COOKIE["auto_signin"] = $_SESSION["auto_signin"];
  };
